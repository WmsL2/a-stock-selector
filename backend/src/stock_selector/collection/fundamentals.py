...
class FinancialCollector:
    ...
    def collect(
        ...
        return _collect_per_symbol(
            ...
            lambda records: _validate_financial(records, start_period, end_period),
...
class ValuationCollector:
    ...
    def collect(
        ...
        return _collect_per_symbol(
            ...
            lambda records: _validate_valuations(records, as_of),
...
class IndustryCollector:
    ...
    def collect(
        ...
        return _collect_per_symbol(
            ...
            lambda records: _validate_industries(records, as_of),
...
def _collect_per_symbol[TRecord: (FinancialRecord, ValuationRecord, IndustryRecord)](
    ...
    validate: Callable[[tuple[TRecord, ...]], tuple[TRecord, ...]],
    ...
    return FundamentalsCollectionReport(
        symbols, succeeded, empty, failed, persisted_rows
    )


def _validate_financial(
    records: tuple[FinancialRecord, ...], start_period: date, end_period: date
...
def _validate_valuations(
    records: tuple[ValuationRecord, ...], as_of: datetime
) -> tuple[ValuationRecord, ...]:
    return _validate_records(
        records, lambda item: item.as_of <= as_of, lambda item: item.as_of
    )


def _validate_industries(
    records: tuple[IndustryRecord, ...], as_of: date
