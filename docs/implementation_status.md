...
## Task 24 Fix Verification

- Audit-linkage closure — PASS: Task24 now retains the exact `FiveFactorCrossSectionResult` and
  `FactorsConfig` used for composition. Normal result construction verifies each BaseScore family
  contribution against the retained factor score, component coverage, configured enablement and
  weight, rejecting same-symbol/same-as-of BaseScore evidence from another valid run.
- Regressions independently reject an incorrect `risk_ready`, operational price factors, mismatched
  factor-input timestamp, unsorted factor inputs, cross-run factor configuration/evidence and
  cross-run BaseScore evidence. DailySelection parity additionally checks retained family evidence
  and configured weights without changing DailySelection.
...
- `GET /api/selection/realtime` resolves one aware point-in-time `as_of`, runs the injected
  Task25 runtime once, and returns only compact Top100 selection evidence. It neither exposes
  the full capture nor persists a snapshot.
- The app lifespan owns optional injected realtime-provider composition. Route dependencies only
  retrieve the configured provider; provider connection failures become a stable 503 response.
- The response retains Task15–22 readiness, blockers, policy, diagnostics and selected-item audit
  fields without adding scoring, ranking, filtering, polling or UI behavior.

## Task 26 Verification

- Focused offline API regressions — PASS (8 tests); the full API suite — PASS (29 tests), and the
  realtime suite — PASS (219 tests). They use injected fake providers and isolated repositories
  only, covering one runtime call, compact mapping, ready/blocked/empty behavior, errors, OpenAPI,
  provider isolation and no snapshot writes.
